...
export function renderTemplate(config: BrandConfig): { html: string; css?: string } {
  ...
  const backgroundColor = (config as any).backgroundColor || '#000000'
...
<body>
  ...
  <iframe 
    id="gameFrame"
    src="https://www.nanokit.io/templates/game/game.html?theme=pirates&url=${encodeURIComponent(ctaUrl || '#')}&popupTitle=${encodeURIComponent(popupTitle)}&popupMessage=${encodeURIComponent(popupMessage)}&popupPrize=${encodeURIComponent(popupPrize)}&wheelValues=${encodeURIComponent(wheelValues)}&logoUrl=${encodeURIComponent(logoUrl)}"
    allowfullscreen
  ></iframe>
