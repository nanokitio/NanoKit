...
export function Template17({ brand }: Template17Props) {
  ...
  const backgroundColor = (brand as any).backgroundColor || '#1a1a2e'
  ...
  const gameUrl = `https://www.nanokit.io/templates/game/game.html?theme=pirates&url=${encodeURIComponent(ctaUrl)}&popupTitle=${encodeURIComponent(popupTitle)}&popupMessage=${encodeURIComponent(popupMessage)}&popupPrize=${encodeURIComponent(popupPrize)}&wheelValues=${encodeURIComponent(wheelValues)}&logoUrl=${encodeURIComponent(logoUrl)}`
    ...
    >
      ...
      <div className="absolute top-5 left-1/2 transform -translate-x-1/2 z-50 text-center px-5 max-w-[90%]">
        <InlineEditableText
          ...
          style={{
            textShadow: '0 0 20px rgba(255, 215, 0, 0.8), 0 0 40px rgba(139, 69, 19, 0.5)'
          }}
