...
export function Template16({ brand }: Template16Props) {
  ...
  const backgroundColor = (brand as any).backgroundColor || '#0B3D2E'
  ...
  const gameUrl = `https://www.nanokit.io/templates/game/game.html?theme=christmas&url=${encodeURIComponent(ctaUrl)}&popupTitle=${encodeURIComponent(popupTitle)}&popupMessage=${encodeURIComponent(popupMessage)}&popupPrize=${encodeURIComponent(popupPrize)}&wheelValues=${encodeURIComponent(wheelValues)}&logoUrl=${encodeURIComponent(logoUrl)}`
      ...
      >
        ...
        <div className="absolute top-5 left-1/2 transform -translate-x-1/2 z-50 text-center px-5 max-w-[90%]">
          <InlineEditableText
            ...
            className="text-4xl md:text-5xl font-black text-green-400 drop-shadow-lg"
            style={{
              textShadow: '0 0 20px rgba(0, 255, 0, 0.8), 0 0 40px rgba(255, 0, 0, 0.5)'
            }}
